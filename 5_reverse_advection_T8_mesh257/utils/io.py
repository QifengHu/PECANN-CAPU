...
def save_st_points(x_dom, y_dom, t_dom, x_bc, y_bc, t_bc, x_ic, y_ic, t_ic, out_dir, trial):
    data_dom = torch.cat((x_dom, y_dom, t_dom), dim=1).detach().cpu().numpy()
    data_bc  = torch.cat((x_bc,  y_bc,  t_bc),  dim=1).detach().cpu().numpy()
    data_ic  = torch.cat((x_ic,  y_ic,  t_ic),  dim=1).detach().cpu().numpy()
...
def append_logs(epoch, state, objective, constr,
    ...
    mu_buf.append(torch.cat([e, state.Mu.cpu().view(-1)]))
    lambda_buf.append(torch.cat([e, state.Lambda.cpu().view(-1)]))
    constr_buf.append(torch.cat([e, constr.cpu().view(-1)]))
    obj_buf.append(torch.cat([e, torch.tensor([objective.cpu().item()])]))
    


def flush_logs(trial, domain_win,
    ...
    with open(os.path.join(out_dir, f"{trial}_{domain_win[-1,-1]:.2f}_mu.dat"), mode) as f:
        np.savetxt(f, mu_arr, fmt="%.6e", delimiter=" ")

    with open(os.path.join(out_dir, f"{trial}_{domain_win[-1,-1]:.2f}_lambda.dat"), mode) as f:
        np.savetxt(f, lambda_arr, fmt="%.6e", delimiter=" ")

    with open(os.path.join(out_dir, f"{trial}_{domain_win[-1,-1]:.2f}_constr.dat"), mode) as f:
        np.savetxt(f, constr_arr, fmt="%.6e", delimiter=" ")

    with open(os.path.join(out_dir, f"{trial}_{domain_win[-1,-1]:.2f}_object.dat"), mode) as f:
