...
def save_spatio_points(x_dom, y_dom, x_bc, y_bc, out_dir, trial):
    data_dom = torch.cat((x_dom, y_dom), dim=1).detach().cpu().numpy()
    data_bc  = torch.cat((x_bc,  y_bc),  dim=1).detach().cpu().numpy()
